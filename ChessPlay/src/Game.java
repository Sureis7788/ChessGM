...
public class Game {
    ...
    public boolean canMove(int x, int y, int destX, int destY, Side s){
       ...
       - piece must move "through" a piece to get from (x,y) to (destX,destY) (use isVisible())
          ...
          */
        if((x<0||x>7||y<0||y>7||destX<0||destX>7||destY<0||destY>7)||(x==destX&&y==destY)){
        return false;
        }
        ...
        else if((b.get(x,y).canMove(destX,destY))==false){
            return false;
        }
        else if((b.get(destX,destY)!=null)&&(b.get(x,y).getSide()==b.get(destX,destY).getSide())){

                return false;

        }
        //knight hop no mather if there are pieces
        else if(b.get(x,y) instanceof Knight){
            return b.get(x,y).canMove(destX,destY);
        }
    ...
    public boolean move(int x, int y, int destX, int destY) {
        ...
        Side oppside=Side.negate(currentTurn);
        King oppking=b.getKing(oppside);
        int kx= oppking.x;
        int ky= oppking.y;
        //validity movement check
        if (!canMove(x, y, destX, destY, currentTurn)) {
            return false;
        }
        else if(canMove(x, y, destX, destY, currentTurn)) {
            //after moved ,the b.get(x,y)==null,so records first
            appendMoveToHistory(x, y, destX, destY, currentTurn);
            boolean moved=b.get(x,y).move(destX,destY);

            if(moved) {
                if(kx==destX && ky==destY){//results in a win
                    appendWinToHistory(currentTurn);}
                else{
                    if(isInCheck(currentTurn)){        // - result in check EDIT: for either side
                    appendCheckToHistory(currentTurn);
                }
                else if(isInCheck(oppside)){
                    appendCheckToHistory(oppside);
                }
                currentTurn=oppside;
    ...
    public boolean isInCheck(Side side) {
        if (b.getKing(side) != null) {
            ...
            int destKx = b.getKing(side).x;
            int destKy = b.getKing(side).y;
            Side oppside = Side.negate(side);
            // get all the opponent side pieces
            List<Piece> P = b.getPieces(oppside);
            for (Piece piece : P) {
                int x = piece.x;
                int y = piece.y;
                if (canMove(x, y, destKx, destKy, oppside)) {
                    return true;
                }
            }
